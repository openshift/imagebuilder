...
func NewStages(node *parser.Node, b *Builder) (Stages, error) {
	...
	setStageInheritedArgs := func(s *Stage) error {
		...
		args := slices.Clone(inheritedArgs)
		...
		if as != "" {
			argInstructionsInStages[as] = args
		}
		argInstructionsInStages[strconv.Itoa(s.Position)] = args
		return arg(s.Builder, args, nil, nil, "", nil)
	}
	var stages Stages
